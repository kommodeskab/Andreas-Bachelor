...
class MarginalDistributionsImagesCB(pl.Callback):
    def __init__(self, num_rows : int = 5):
        """
        Callback to log images from the marginal distributions of the data and prior.
        Only works for images. Visualizes examples from the start and end datasets.
        """
        super().__init__()
...
class PlotImageSamplesCB(pl.Callback):
    def __init__(
        ...
        """
        Plots samples from the model on the validation set.
        On the left, the original images, on the right, the samples.
        """
        super().__init__()
        self.num_rows = num_rows
...
class SanityCheckImagesCB(pl.Callback):
    def __init__(self):
        """
        Callback to check if the model can reverse the process.
        Even if the model have converged, it might not be a solution to the SBP.
        Instead, it should be able to reverse opposite process. 
        Therefore, this callback shows 3 rows of images:
        - The original data
        - The prior
        - The reconstruction
        """
    ...
    def on_validation_end(self, trainer: pl.Trainer, pl_module: Union[FRDSB, TRDSB] ) -> None:
        ...
        cmap = "gray" if data.shape[3] == 1 else None
        
        fig, axs = plt.subplots(3, 5, figsize=(20, 12))
        for i in range(5):
            axs[0, i].imshow(data[i], cmap=cmap)
            axs[1, i].imshow(prior[i], cmap=cmap)
            axs[2, i].imshow(reconstruction[i], cmap=cmap)
...
class CalculateFID(pl.Callback):
    def __init__(self, num_samples : int = 1000, ema_scope : bool = True):
        """
        Callback to calculate the FID between the start and end datasets.
        The FID is calculated between the original images and the images generated by the model.
        """
        super().__init__()
        self.num_samples = num_samples
        self.ema_scope = ema_scope
        self.have_calculated = {}
    ...
    def on_train_epoch_end(self, trainer: pl.Trainer, pl_module: StandardDSB) -> None:
        ...
        if not self.have_calculated.get(iteration, False) and not training_backward:
            t1 = time.time()
            pl_module.eval()
            ...
            print(f"FID calculation took {time.time() - t1:.2f} seconds.")
            self.have_calculated[iteration] = True
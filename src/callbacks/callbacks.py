...
class PlotGammaScheduleCB(pl.Callback):
    ...
    def on_train_start(self, trainer: pl.Trainer, pl_module: StandardDSB) -> None:
        ...
        gammas_fig = get_gamma_fig(gammas, "Gamma", "Gamma schedule")

        gammas_bar = pl_module.gammas_bar[1:]
        gammas_bar_fig = get_gamma_fig(gammas_bar, "Gamma bar", "Gamma bar schedule")

        sigma_backward = pl_module.sigma_backward[1:]
        sigma_backward_fig = get_gamma_fig(sigma_backward, "Sigma backward", "Sigma backward schedule")

        sigma_forward = pl_module.sigma_forward[1:]
        sigma_forward_fig = get_gamma_fig(sigma_forward, "Sigma forward", "Sigma forward schedule")
...
class MMDCB(pl.Callback):
    ...
    def on_train_epoch_end(self, trainer: pl.Trainer, pl_module: StandardDSB) -> None:
        ...
        if not pl_module.hparams.training_backward:
            ...
            logger.log_metrics({"benchmarks/MMD": mmd_value}, step=trainer.global_step)

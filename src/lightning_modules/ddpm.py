...
class DDPM(BaseLightningModule):
    ...
    def sample_timesteps(self, batch_size : int):
        return torch.randint(
            0, 
            self.hparams.num_train_timesteps, 
            (batch_size,)
            ).to(self.device)
    ...
    def _common_step(self, batch):
        ...
        loss = self.mse(model_output, batch)
        return loss
    ...
    def sample(self, noise : torch.Tensor):
        batch_size = noise.size(0)
        sample = noise
        for t in self.scheduler.timesteps:
            timestep = self.t_to_tensor(t, batch_size)
            model_output = self.model(sample, timestep)
            sample = self.scheduler.step(model_output, timestep, sample)
            
        return sample
    
    def configure_optimizers(self):
        optim = self.partial_opt(self.model.parameters())
        sch = ReduceLROnPlateau(optim, mode="min", factor=0.5, patience=10, verbose=True)
        return {
            "optimizer" : optim,
            "lr_scheduler" : sch,
            "monitor" : "val_loss",
